...
export function createCylinder(params: CylinderParams = {}): MeshData {
  ...
  for (let y = 0; y <= hs; y++) {
    ...
    for (let x = 0; x <= rs; x++) {
      ...
      pos.push(r * cosT, py + (x / rs) * rise, r * sinT)
      if (rise === 0) nrm.push(nRadial * cosT, nY, nRadial * sinT)
      uvArr.push(x / rs, 1 - t)
    }
  }
  for (let y = 0; y < hs; y++) {
  ...
  if (rise === 0) {
    if (radiusTop > 0) {
      ...
      for (let x = 0; x < rs; x++) idx.push(base, base + 1 + x + 1, base + 1 + x)
    }
    if (radiusBottom > 0) {
      const base = pos.length / 3
      pos.push(0, -halfH, 0); nrm.push(0, -1, 0); uvArr.push(0.5, 0.5)
      ...
      for (let x = 0; x < rs; x++) idx.push(base, base + 1 + x, base + 1 + x + 1)
    }
  }
